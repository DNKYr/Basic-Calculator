...
if __name__ == "__main__":    repeat = True    print_welcome_message()    while repeat:        try:             first_number = int(input("Please input the first number \n"))            operation_type = str(input("Now please input the operation you want to perform (eg. +, -, *, /)\n"))            second_number = int(input("Please input the second number\n"))            result = calculate(first_number, second_number, operation_type)            print(result)        except ZeroDivisionError as e:            print(f"ZeroDvisionError: Division by Zero is undefined")        except ValueError as e:            print(f"ValueError: {e}")                r = str(input("Do you want to end this program? If yes please enter: STOP \n"))        if r.upper() == "STOP":            repeat = False    print_outro_message()